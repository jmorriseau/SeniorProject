<?php
/**
 * Class that contains the resources for making any changes to the 'Building' table in the database.
 *
 * @author Mike
 */
header('Content-type: application/json');
include('../autoload.php');

$data = array();
$dbc = new DAO();

$success = true;
$response_array['status'] = 'success';
$db_success = '';
$err_msg = '';
$message;

switch($_SERVER['REQUEST_METHOD']){
 case 'GET':
   $id;
   if($_GET['id'] === ''){
     $id = NULL;
   }
   else{
     $id = $_GET['id'];
   }
   $message = courseAttrResourceRun('GET', $id , NULL, $dbc);
   break;

 case 'POST':

    $data['attributes_id'] = $_POST['attributesId'];
    $data['course_id'] = $_POST['courseId'];
    $message = courseAttrResourceRun('POST', NULL, $data, $dbc);
    break;

 case 'PUT':
    $data['attributes_id'] = $_POST['attributesId'];
    $data['course_id'] = $_POST['courseId'];
    $id = $_POST['id'];
    $message = courseAttrResourceRun('PUT', $id, $data, $dbc);
    break;

 case 'DELETE':
   $id = $_GET['id'];
   $message = courseAttrResourceRun('DELETE', $id, NULL, $dbc);
   break;
}

echo json_encode($message);
/*
* API Resources Function for Buildings
* REST SERVER CALLS FOR BUILDING RESOURCES
*/

function courseAttrResourceRun($verb, $id = NULL, $inputData = NULL, $db){
  $returnMessage;
  if ( 'GET' === $verb ) {
     if ( NULL === $id ) {
       $returnMessage = getAll($db);
     } else {
       $returnMessage = get($id,$db);
     }
  }

  if ( 'POST' === $verb ) {
     if ($inputData === NULL) {
         throw new Exception('Course Attribute could not be added');
     } else {
        $returnMessage = post($inputData,$db);
     }
  }

  if ( 'PUT' === $verb ) {
     //if put is the verb, check the ID. If null, throw an exception looking for a id.
     if ( NULL === $id && $inputData === NULL) {
         throw new InvalidArgumentException('Course Attribute ID ' . $id . ' was not found');
     } else {
         //if not, run the put function and set the message to the return.
         $returnMessage = put($id,$inputData,$db);
     }
  }

  if ('DELETE' === $verb) {
     //if the delete verb is selected, check the the id is not null, and if so, throw an exception.
     if ( NULL === $id ) {
         throw new InvalidArgumentException('Course Attribute ID ' . $id . ' was not found');
     } else {
         // if its all clear, set the message to the delete function return.
       $returnMessage = delete($id,$db);
     }
  }
  // IMPORTANT!!! Return coming up.
  return $returnMessage;
}

function getAll($db) {
    //Very similar statement to get(), except this function requires no parameters and returns all entries in the db.
    return $db->sql("SELECT Course_Attribute_Relation.course_id, Courses.course_name, Course_Attribute_Relation.attributes_id, Attributes.attributes_name from Courses, Attributes, Course_Attribute_Relation WHERE Course_Attribute_Relation.course_id = Courses.course_id AND Course_Attribute_Relation.attributes_id = Attributes.attributes_id;");
    //return $this->db;
}

function get($id,$db) {
    //Creates statement used to get specific entry from the database based on an id given via endpoint
    return $db->sql("SELECT Course_Attribute_Relation.course_id, Courses.course_name, Course_Attribute_Relation.attributes_id, Attributes.attributes_name from Courses, Attributes, Course_Attribute_Relation WHERE Course_Attribute_Relation.course_id = Courses.course_id AND Course_Attribute_Relation.attributes_id = Attributes.attributes_id AND course_attr_relation_id ='".$id ."'");
}

function post($data,$db)
{
    //Setups up the insert for SQL for post function as well as binding JSON data provided by the data array to the statement

    try{
       $db->sql("INSERT INTO Course_Attribute_Relation (
         attributes_id, course_id)
         VALUES(
       '" .$data['attributes_id']. "',
       '" .$data['course_id']. "')
      ;");
      return 'Course Attribute Added';
    }
    catch(Exception $e){
      //return new Exception($e);
      //return 'INSERT not accepted';
      return $e->getMessage();
    }

}


function put($id,$data,$db)
{
    //Put function uses a statement written to update a pre-existing db entry.
    try {
      $db->sql("Update Course_Attribute_Relation SET attributes_id ='" .$data['attributes_id'] ."'
      , course_id = '". $data['course_id'] . "'
      WHERE course_attr_relation_id = '" . $id . "'");
        return 'Course Attribute Updated';
    } catch (Exception $e){
            throw new Exception('Course Attribute could not be updated');
    }
}

function delete($id,$db) {
    //Delete uses a statment written to delete from the db where the id matches the one located in the endpoint.
    $db->sql("DELETE FROM Course_Attribute_Relation WHERE course_attr_relation_id = '".$id."';");

    if($db->sql("select * from Course_Attribute_Relation where course_attr_relation_id ='".$id."';").length == 0)
    {
      return "Course Attribute Deleted";
    }  else {
      return "Error Course Attribute not Deleted";
    }

}

    /*public function dataCheck($data) {
        //The dataCheck function uses the JSON data to make sure that all form objects were properly filled out.
        $errors = array();

        if ($data['campus_id'] === '' ){
            $errors[] = 'No Campus ID ';
        }
        if ($data['building_name'] === '' ){
            $errors[] = 'No Building Name ';
        }
        if ($data['building_abbreviation'] === '' ){
            $errors[] = 'No Abbreviation ';
        }
        if (count($errors) > 0)
        {
            throw new Exception('Form not fully filled');
        }
        else{
            return true;
        }

    }*/
